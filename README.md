...
Currently the Moonbeam team is perfecting a few small kinks in compatability, so we're going to use some slightly modified code to talk to the node. Simply copy and paste the code [from this file](https://github.com/evan-forbes/DictatorToken/blob/master/private-provider.js) in a file called `private-prodiver.js` in the root of your project's directory.
